...
def ref_alignment(supports, masses, ref_index=0):
    ...
    pis = [emd(ref_mass, mui_mass, cdist(ref_supp, mui_pos, metric='sqeuclidean'), numItermax=1_00_000)
...
def pairwise_kernels(supports, masses):
    ...
    for i in range(n):
        for j in range(i+1, n):
            pi_ij = emd(masses[i], masses[j], cdist(supports[i], supports[j], metric='sqeuclidean'), numItermax=1_000_000)
